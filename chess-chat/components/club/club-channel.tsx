...
import { Channel, ChannelType, Club, MemberRole } from "@prisma/client";
...
export const ClubChannel = ({ channel, club, role }: ClubChannelProps) => {
  ...
  const onClick = () => {
    router.push(`/clubs/${params?.clubId}/channels/${channel.id}`);
  };

  const onAction = (e: React.MouseEvent, action: ModalType) => {
    e.stopPropagation();
    onOpen(action, { channel, club });
  };
        ...
        <div className="ml-auto flex items-center gap-x-2">
          <ActionTooltip label="Edit" side={"top"} align={"end"}>
            <Edit
              ...
              className="hidden group-hover:block w-4 h-4 text-zinc-500 hover:text-zinc-400 dark:hovertext-zinc-300 transsition"
            />
          </ActionTooltip>
