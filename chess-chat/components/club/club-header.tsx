  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-neutral-400 space-y-[2px]">
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={() => onOpen("members", { club })} className=" px-3 py-2 text-sm cursor-pointer">
            Manage Members
        ...
        {isAdmin && (
          <DropdownMenuItem onClick={() => {onOpen("deleteClub", {club})}} className="text-rose-500 px-3 py-2 text-sm cursor-pointer">
            Delete Club
            <Trash className="h-4 w-4 ml-auto" />
          </DropdownMenuItem>
        )}
        {!isAdmin && (
          <DropdownMenuItem onClick={()=> onOpen("leaveClub", {club})} className="text-rose-500 px-3 py-2 text-sm cursor-pointer">
            Leave Club
